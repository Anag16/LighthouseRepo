...
describe("#middle", () => {
  it("returns true for [2,5,4,7,7]), [4]", () => {
    assert.strictEqual(assertEqual(eqArrays(middle([2,5,4,7,7]), [4]), true));
  });

  it("returns true for ['A','B','C','E','F']), ['C']", () => {
    assert.strictEqual(assertEqual(eqArrays(middle(['A','B','C','E','F']), ['C']), true));
  });

  it("returns true for([2,5,4]), [5])", () => {
    assert.strictEqual(assertEqual(eqArrays(middle([2,5,4]), [5]), true));
  });
